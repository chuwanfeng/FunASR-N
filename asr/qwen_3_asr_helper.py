...
def convert_qwen3_asr_model(
    model_id, output_dir, quantization_config=None, use_local_dir=False
):
    """
    ...
    if not TORCH_AVAILABLE:
        raise ImportError(
            "PyTorch is required for model conversion. Please install torch."
        )
    ...
    if all(
        ...
        print(
            f"✅ {model_id} model already converted. You can find results in {output_dir}"
        )
        return
    ...
    if use_local_dir:
        ...
        if not ckpt.exists():
            ...
            # 如果是本地路径，直接跳过下载（魔搭的下载函数不支持本地路径作为 source）
            import os
            if os.path.isdir(model_id):
                print(f"📂 检测到本地路径，跳过下载步骤: {model_id}")
            # 确保缓存目录存在并填入文件（简单的复制逻辑）
            import shutil
            os.makedirs(ckpt, exist_ok=True)
            for item in os.listdir(model_id):
                s = os.path.join(model_id, item)
                d = os.path.join(ckpt, item)
                if os.path.isdir(s): shutil.copytree(s, d, dirs_exist_ok=True)
                else: shutil.copy2(s, d)
        else:
            # 如果是远程ID，使用魔搭下载
            print(f"🚀 使用 ModelScope 下载模型: {model_id}")
            ms_download(model_id, local_dir=ckpt, revision='master')
    else:
        ckpt = model_id
    ...
    model = Qwen3ASRForConditionalGeneration.from_pretrained(
        ckpt, config=config, torch_dtype=torch.float16
    )
    model.eval()
    ...
    if not thinker_audio_path.exists():
        ...
        ov_model = ov.convert_model(
            audio,
            example_input={
                "padded_feature": torch.randn(
                    [3, num_mel_bins, 100], dtype=torch.float32
                ),
            },
    ...
    if not thinker_lang_path.exists():
        ...
        ov_model = ov.convert_model(
            lang_model, example_input=example_input, input=input_shapes
        )
        ...
        gc.collect()
        print(
            f"✅ Thinker model conversion finished. You can find results in {output_dir}"
        )
...
class SinusoidsPositionEmbedding:
    ...
    def __init__(self, length: int, channels: int, max_timescale: float = 10000.0):
        if channels % 2 != 0:
            raise ValueError(
                "SinusoidsPositionEmbedding requires even number of channels"
            )
...
class OVQwen3ASRThinkerForConditionalGeneration(GenerationMixin):
    ...
    def __init__(self, model_dir, device, config):
        ...
        self.audio_conv = ov.Core().compile_model(
            self.model_dir / THINKER_AUDIO_NAME, device
        )

        print(
            f"⌛ Loading audio encoder model from {self.model_dir / THINKER_AUDIO_ENCODER_NAME}"
        )
        self.audio_encoder = ov.Core().compile_model(
            self.model_dir / THINKER_AUDIO_ENCODER_NAME, device
        )

        # Load embedding model
        print(
            f"⌛ Loading embedding model from {self.model_dir / THINKER_EMBEDDING_NAME}"
        )
        self.embed_tokens_model = ov.Core().compile_model(
            self.model_dir / THINKER_EMBEDDING_NAME, device
        )

        # Load language model (stateful)
        print(
            f"⌛ Loading language model from {self.model_dir / THINKER_LANGUAGE_NAME}"
        )
    ...
    def audio_tower(
        ...
        tail_chunk_index = torch.nn.functional.pad(chunk_num, (1, 0), value=-1).cumsum(
            0
        )[1:]
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits = torch.from_numpy(self.request.get_tensor("logits").data.copy()).to(
            self.device
        )
...
class OVQwen3ASRModel:
    ...
    def __init__(
        ...
        if not INFERENCE_UTILS_AVAILABLE:
            raise ImportError(
                "Qwen3-ASR inference utilities not available. Please install qwen_asr package."
            )

        if not GENERATION_MIXIN_AVAILABLE:
            raise ImportError(
                "GenerationMixin not available. Please install transformers>=4.40."
            )
    ...
    def get_support_languages(self) -> List[str]:
        ...
        return (
            self.config.support_languages
            if hasattr(self.config, "support_languages")
    ...
    def _infer_asr(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            inputs = self.processor(
                text=sub_text, audio=sub_wavs, return_tensors="pt", padding=True
            )
    ...
    def transcribe(
        ...
        for i in range(n):
            ...
            results.append(
                ASRTranscription(
                    language=merged_language, text=merged_text, time_stamps=None
                )
            )
